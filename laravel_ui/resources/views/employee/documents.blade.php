...
function openViewer(id, title) {
    ...
    document.getElementById('viewer-body').innerHTML = '<div style="text-align:center;padding:2rem;color:#9bb0b0;">Chargement…</div>';
    ...
    .then(data => {
        if (data.content) {
            document.getElementById('viewer-body').textContent = data.content;
        } else if (data.error) {
            document.getElementById('viewer-body').innerHTML = '<span style="color:#dc2626;">' + data.error + '</span>';
        }
    })
    .catch(() => {
        document.getElementById('viewer-body').innerHTML = '<span style="color:#dc2626;">Impossible de charger le document.</span>';
    });
}
